...
u32 ElfReader::MappedSize() const {
  u32 lo_addr = 0xffffffff, hi_addr = 0;
  ...
  for (int i = 0; i < header->e_phnum; i++)
  {
    ...
    if (p->p_type == PT_LOAD)
    {
      lo_addr = std::min(p->p_vaddr, lo_addr);
      hi_addr = std::max(p->p_vaddr + p->p_memsz, hi_addr);
    }
  }

  return lo_addr < hi_addr ? hi_addr - lo_addr : 0;
}

bool ElfReader::LoadIntoMemory(Core::System& system, bool only_in_mem1) const
{
  return LoadIntoMemory(system, 0, only_in_mem1);
}

// This is just a simple elf loader, good enough to load elfs generated by devkitPPC
bool ElfReader::LoadIntoMemory(Core::System& system, u32 slide, bool only_in_mem1) const
{
  ...
  for (int i = 0; i < header->e_phnum; i++)
  {
    ...
    INFO_LOG_FMT(BOOT, "Type: {} Vaddr: {:08x} Filesz: {} Memsz: {}", p->p_type, p->p_vaddr + slide,
                 p->p_filesz, p->p_memsz);

    if (p->p_type == PT_LOAD)
    {
      u32 writeAddr = p->p_vaddr + slide;
      const u8* src = GetSegmentPtr(i);
      u32 srcSize = p->p_filesz;
      u32 dstSize = p->p_memsz;

      if (only_in_mem1 && (p->p_vaddr + slide) >= memory.GetRamSizeReal())
        continue;
...
std::unordered_map<std::string_view, u8 const*> ElfReader::StaticSymMap() const
{
  std::unordered_map<std::string_view, u8 const*> sym_map;

  SectionID sec = GetSectionByName(".symtab");
  if (sec != -1)
  {
    int strings_idx = sections[sec].sh_link;
    const char* strings_base = (const char*)GetSectionDataPtr(strings_idx);

    Elf32_Sym* symtab = (Elf32_Sym*)(GetSectionDataPtr(sec));
    int nsyms = sections[sec].sh_size / sizeof(Elf32_Sym);
    for (int sym = 0; sym < nsyms; sym++)
    {
      ...
      const int sec_index = Common::swap16(symtab[sym].st_shndx);
      const u32 value = Common::swap32(symtab[sym].st_value);
      const char* name = strings_base + Common::swap32(symtab[sym].st_name);
      if (sections[sec_index].sh_type == SHT_NOBITS)
      {
        // Signifies NOBITS
        sym_map[std::string_view(name)] = nullptr;
      }
      else
      {
        const u32 rva = value - sections[sec_index].sh_addr;
        if (rva < sections[sec_index].sh_size)
        {
          sym_map[std::string_view(name)] = GetSectionDataPtr(sec_index) + rva;
        }
      }
    }
  }
                            ...
                            const std::string& filename, u32 slide) const
{
  ...
  if (sec != -1)
  {
    ...
    for (int sym = 0; sym < numSymbols; sym++)
    {
      ...
      ppc_symbol_db.AddKnownSymbol(guard, value + slide, size, name, filename, symtype);
      hasSymbols = true;
    }
  }
  ppc_symbol_db.Index();
...
{
  return LoadSymbols(guard, ppc_symbol_db, filename, 0);
}
