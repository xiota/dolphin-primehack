...
void ElfModLoader::parse_and_load_modfile(std::string const& path) {
  ...
  bool resolved = resolve_symbols(
                  [this] (const Symbol* callgate_fn_sym,
                          const Symbol* dispatch_table_sym,
                          const Symbol* callgate_dispatch_fn_sym,
                          const Symbol* trampoline_restore_table_sym) {
  ...
  resolved = resolve_symbols([this] (const Symbol* release_fn_sym, const Symbol* shutdown_signal_sym) {
  ...
  for (CVar const& cvar : parsed_cvars) {
    resolve_symbols([this, &cvar] (const Symbol* cvar_sym) {
  ...
  for (auto&& [hook_fn, hook_addr] : parsed_vthooks) {
    resolve_symbols([this, hook_addr = hook_addr] (const Symbol* hook_sym) {
  ...
  debug_output_addr = 0;
  resolve_symbols([this] (const Symbol* debug_out_sym) {
