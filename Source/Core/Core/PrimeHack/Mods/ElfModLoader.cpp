...
auto resolve_symbols(T&& cb, std::string const& name, Args const&... tnames)
  ...
  auto symbols = std::make_tuple(ppc_symbol_db.GetSymbolFromName(name), ppc_symbol_db.GetSymbolFromName(tnames)...);
...
bool ElfModLoader::load_elf(std::string const& path) {
  ...
  if (elf_file.IsValid()) {
    ...
    ppc_symbol_db.Clear();
    elf_file.LoadSymbols(*active_guard, ppc_symbol_db);
  }
